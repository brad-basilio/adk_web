...
const FloatingButtons = ({ generals = [] }) => {
  // Obtener el teléfono de soporte desde generals
  const getGeneralValue = (correlative) => {
    const general = generals?.find(g => g.correlative === correlative);
    return general?.description || '';
  };

  const supportPhone = getGeneralValue('support_phone') || getGeneralValue('phone_contact')?.split(',')[0]?.trim() || '+1234567890';
  
  // Limpiar el número de teléfono para el href
  ...
  return (
    <div className="floating-buttons">
      {/* Botón de Chat - redirige a Contact */}
      <motion.button
